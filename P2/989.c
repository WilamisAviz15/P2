...
deque_t* construct    ()
{
  deque_t *deck = (deque_t*) malloc(sizeof(deque_t));
  deck->front = NULL;
  deck->rear  = NULL;
  deck->size = 0;
  return deck;
...
void     enqueue_rear (deque_t *deque, int value)
{
  ...
  else
  {
    ...
    {
      novo->prev = deque->rear;
      deque->rear->next = novo;
      deque->rear = novo;
    }
    deque->size++;
  }
...
void     enqueue_front(deque_t *deque, int value)
{
    ...
    else
    {
      ...
      {
        novo->next = deque->front;
        deque->front->prev = novo;
        deque->front = novo;
      }
      deque->size++;
    }
